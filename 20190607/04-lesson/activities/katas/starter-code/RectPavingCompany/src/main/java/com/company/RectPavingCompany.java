...
public class RectPavingCompany {
    ...
    public static void main(String[] args) {
        ...
        framingCost = Float.parseFloat(scan.nextLine()); //23 - changed the variable that is assigned the price of framing,  from cementCost to framingCost

        int area = length * width;
        int perimeter = 2*length + 2*width; //26 - changed the subtraction to addition
        ...
        System.out.format("The cost of the cement is $%.2f\n", area*cementCost); //31 - added a dollar sign and decimal format
        System.out.format("The cost of the framing/footers is $%.2f", perimeter*framingCost); //32 - changed the addition to multiplication and added a dollar sign and decimal format
